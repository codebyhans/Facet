...
class AlbumQueryCacheService:
    ...
    def invalidate_all(self) -> None:
        ...
        try:
            self._session.execute(delete(AlbumQueryCacheModel))
            self._session.flush()
        except Exception as exc:
    ...
    def _cache_is_valid(self, album: Album) -> bool:
        ...
        try:
            ...
            versions = {int(value) for value in self._session.scalars(stmt)}
    ...
    def _cached_image_ids(self, album_id: int) -> list[int]:
        try:
            ...
            return [int(image_id) for image_id in self._session.scalars(stmt)]
        except Exception as exc:
